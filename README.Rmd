...
sys_lapply <- sys$results[sys$results$method == "lapply", , drop = FALSE]
mtl_lapply <- mtl$results[mtl$results$method == "lapply", , drop = FALSE]
...
baseline_sys <- sys_lapply$median_seconds[[1L]]
baseline_mtl <- mtl_lapply$median_seconds[[1L]]

tab$speedup_vs_sys_lapply <- baseline_sys / tab$median_seconds
tab$speedup_vs_mtl_lapply <- baseline_mtl / tab$median_seconds

tab <- tab[order(tab$build, tab$method, tab$threads), c(
...
cat(sprintf("mtl lapply / system lapply = %.3fx\n", baseline_mtl / baseline_sys))
```
