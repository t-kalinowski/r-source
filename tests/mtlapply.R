...
pool_before <- .Internal(mtlpoolstats(FALSE))
invisible(mtlapply_with_threads(4L, 1:12, function(i) {
  unlist(mtlapply(1:3, function(j) i + j), use.names = FALSE)
}))
pool_after <- .Internal(mtlpoolstats(FALSE))
stopifnot(pool_before[["threads.current"]] >= 1L)
...
bad <- try(lapply(1:10, function(i) i, threads = 8L), silent = TRUE)
